...
orders.get("/allorders", checkToken, async (req, res) => {
  ...
  try {
    ...
    const [ordersData] = await pool.promise().query(
      `select orderId, DATE_FORMAT(orderDate, '%m/%d/%Y') as
      orderDate, userName, address, city, state, pin, total from orders where userId = ?`,
      [userId],
    );
